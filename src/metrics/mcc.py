"""
Mean Correlation Coefficient (MCC) and R² metrics.

Adapted from icebeem: https://github.com/ilkhem/icebeem/blob/master/metrics/mcc.py

Supports both NumPy (CPU) and PyTorch (GPU) implementations.
PyTorch is optional - if not installed, only NumPy methods will be available.
"""
...
# ============================================================================
# BaseMetric-compatible interface
# ============================================================================
...
class MCCMetric(BaseMetric):
    ...
    Uses the icebeem implementation above.

    MCC finds the optimal permutation that maximizes the mean absolute correlation
    between matched pairs of factors and codes. Supports multiple correlation methods:
    - pearson: Pearson's linear correlation
    - spearman: Spearman's rank correlation (monotonic relationships)
    - rdc: Randomized Dependence Coefficient (nonlinear relationships)
    ...
    def compute_from_matrix(self, R: np.ndarray) -> MetricResult:
        """
        Compute MCC from a precomputed correlation matrix.

        Args:
            R: Absolute correlation matrix of shape (d, m).
        """
        self._validate_matrix(R)

        # Use Hungarian algorithm directly on the correlation matrix
        row_ind, col_ind = linear_sum_assignment(-R)

        # Compute mean of matched correlations
        d, m = R.shape
        k = min(d, m)
        matched_corrs = [R[row_ind[i], col_ind[i]] for i in range(k)]
        mcc_score = float(np.mean(matched_corrs))
